...
# data_path = Path('/reduction/erickoch/M31/sma_12CO/per_mosaic')
...
for this_target_path in all_target_paths:
    ...
    # this_target_ms = [str(filename) for filename in this_target_path.glob("*.ms.lsrk")]
    ...
    if (this_target_path / f"{this_target}_co21_deconv_cube.image").exists():
        if skip_existing:
            print(f"Existing image found. Skipping")
            continue
        else:
            rmtables(str(this_target_path / f"{this_target}_co21_deconv_cube.*"))


    # Setup is for Brick A Column 8 only.
    tclean(vis=this_target_ms,
           imagename=str(this_target_path / f"{this_target}_co21_deconv_cube"),
    ...
    impbcor(str(this_target_path / f"{this_target}_co21_deconv_cube.image"),
            pbimage=str(this_target_path / f"{this_target}_co21_deconv_cube.pb"),
            outfile=str(this_target_path / f"{this_target}_co21_deconv_cube.image.pbcor"))

    immoments(str(this_target_path / f"{this_target}_co21_deconv_cube.image.pbcor"),
              moments=[8],
              outfile=str(this_target_path / f"{this_target}_co21_deconv_cube.image.pbcor.tpeak"))
